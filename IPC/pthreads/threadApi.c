...
_pthread_t *get_blocked_thread_from_pool(blocked_pool_t *block_pool) {
  int i;
...
void dump_block_pool(blocked_pool_t *block_pool) {
  printf("pool size = %d\n", block_pool->pool_size);
  unsigned int i = 0;
  for (; i < block_pool->pool_size; i++) {
    if (block_pool->blocked_thread_collection[i]) {
      printf("pool[%d] = %d\n", i,
             block_pool->blocked_thread_collection[i]->selfid);
...
int is_thread_in_block_pool(unsigned int thid, blocked_pool_t *block_pool) {
  int i;
...
int is_thread_in_block_pool_mutex(unsigned int thid,
...
void init_blocked_pool(blocked_pool_t *block_pool, unsigned int pool_size) {
  int i = 0;
...
int get_empty_slot_from_pool_mutex(blocked_pool_t *block_pool) {
  int i;
  pthread_mutex_lock(&(block_pool->pool_mutex));
  for (i = block_pool->pool_size - 1; i >= 0; i--) {
    if (block_pool->blocked_thread_collection[i] == NULL)
...
int remove_thread_from_pool(blocked_pool_t *block_pool, _pthread_t *thread) {
  int i = 0, loc = -1;
  if (thread->selfid > (block_pool->pool_size - 1))
    return FAILURE;
  pthread_mutex_lock(&(block_pool->pool_mutex));
...
static int _pool_add(blocked_pool_t *block_pool, _pthread_t *thread) {
  int loc = -1;
  if (thread->selfid > block_pool->pool_size - 1)
    return -1;
...
int add_thread_to_pool(blocked_pool_t *block_pool, _pthread_t *thread) {
  if (!block_pool)
    return -1;
  int rc = SUCCESS;
  pthread_mutex_lock(&block_pool->pool_mutex);
...
void pthread_init(_pthread_t *_pthread, unsigned int tid,
...
void wait(_pthread_t *thread_to_block, pthread_mutex_t *mutex,
...
void signal(_pthread_t *signalling_thread, _pthread_t *signalled_thread,
            unsigned int line_no) {
  ...
  if (pthread_cond_signal(&(signalled_thread->cond))) {
    printf("pthread_cond_signal signalling failed, sender_thread id = %d, "
           ...
           signalling_thread->selfid, signalled_thread->selfid, line_no);
    pthread_exit(NULL);
  }
