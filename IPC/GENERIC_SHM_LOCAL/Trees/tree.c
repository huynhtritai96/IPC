...
unsigned int tree_get_node_size() { return sizeof(tree_node_t); }
...
static void *add_tree_node(tree_t *tree, tree_node_t *node) {
  ...
  while (root) {
    parent = root;
    root = (tree_key_compare(node->data, root->data, tree->key_size) < 0)
               ? root->left
               : root->right;
  } // while ends
...
void *tree_init_node(void *key, unsigned int key_size,
...
static tree_node_t *_tree_lookup(tree_node_t *root, void *key, int key_size) {
  if (!root)
    return NULL;
