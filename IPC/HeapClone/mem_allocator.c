...
static void
allocator_add_block_before_current_block(block_meta_data_t *curr_block,
...
static void
allocator_add_block_after_current_block(block_meta_data_t *curr_block,
                                        block_meta_data_t *new_block) {
...
static void
allocator_add_block_to_free_block_list(block_meta_data_t *block_list,
  ...
  for (curr_block = block_list->next_block_pq; curr_block;
       curr_block = curr_block->next_block_pq) {

    prev_block = curr_block;
    if (curr_block->block_size >= block_to_add->block_size)
      continue;
...
allocator_deque_block_from_free_block_list(block_meta_data_t *block_list) {
...
void allocator_init(void *base_address, uint32_t size) {
  ...
  allocator_add_block_to_free_block_list(&vm_page_hdr->free_block_pq_head,
                                         &vm_page_hdr->block);
}

static bool allocator_split_free_data_block_for_allocation(
    vm_page_hdr_t *vm_page, block_meta_data_t *block_meta_data, uint32_t size) {
  ...
  allocator_add_block_to_free_block_list(&vm_page->free_block_pq_head,
                                         next_block_meta_data);
  mm_bind_blocks_for_allocation(block_meta_data, next_block_meta_data);
  return true;
}

void *allocator_alloc_mem(void *base_address, uint32_t req_size) {
  ...
  if (!block_meta_data)
    return NULL;
  assert(block_meta_data->is_free);
  if (block_meta_data->block_size < req_size)
    return NULL;
...
static void allocator_free_block(block_meta_data_t *to_be_free_block) {
  ...
  vm_page_hdr_t *vm_page = MM_GET_PAGE_FROM_META_BLOCK(to_be_free_block);
  ...
  if (next_block) {
    ...
    char *end_address_of_vm_page =
        (char *)((char *)vm_page + vm_page->page_size);
...
void allocator_free_mem(void *addr) {
...
bool allocator_is_vm_page_empty(void *base_address) {

  vm_page_hdr_t *vm_page = (vm_page_hdr_t *)base_address;

  if ((vm_page->page_size !=
       (vm_page->block.block_size + sizeof(block_meta_data_t) +
        sizeof(vm_page->free_block_pq_head) + sizeof(vm_page->page_size))) ||
  ...
  block_meta_data_t *block_meta_data =
      vm_page->free_block_pq_head.next_block_pq;

  assert(block_meta_data);

  if (block_meta_data != &vm_page->block)
    return false;
...
void allocator_print_vm_page(void *base_address) {}

#ifdef MEM_ALLOCATOR_STANDALONE
