  ...
  do {
    ...
    if (C_S.n_concurrent_threads < LIMIT_BARRIER) {
      ...
      if (C_S.n_concurrent_threads < LIMIT_BARRIER) {
        ...
        if (next_thread)
          printf("thread returned from block pool = %d\n", next_thread->selfid);
        if (next_thread) {
      ...
      pthread_mutex_unlock(&C_S.n_concurrent_threads_mutex);
      printf("thread %ld is blocking itself as number of concurrent threads = "
             "%d\n",
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < N_THREADS; i++)
    pthread_create(&(threads[i].pthread_handle), &threads[i].attr, CS,
                   (void *)i);
