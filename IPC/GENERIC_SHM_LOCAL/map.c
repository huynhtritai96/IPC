...
int add_value_in_physical_memory(char *mmap_key, void *value,
  ...
  void *shm_reg = mmap(
      NULL, // let the kernel chose to retunr the base address of shm memory
      size, // sizeof the physical memory to map to virtual address space of the
            // process
...
void *get_value_from_physical_memory(shm_ds_t *ds, void *node,
  ...
  *value_size = *(unsigned int *)(mmap_key + MMAP_KEY_SIZE);
  ...
  shm_reg =
      mmap(NULL, *value_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
