...
void shm_set_dump_fn(shm_ds_t *shm_ds, _dump dump) { shm_ds->dump = dump; }
...
static void *shm_augment_node(void *node, int sizeofnode,
...
int shm_add_elem(shm_ds_t *ds, void *key, unsigned int key_size, void *value,
  ...
  add_value_in_physical_memory((char *)aug_node + ds->get_node_size(), value,
                               struct_id);
  return 0;
}
