...
typedef struct resource {
  ...
  pthread_mutex_t Nreader_count_mutex; // mutex for Nreader_count variable as it
                                       // is accessed by reader(s) and writer
} resource_t;
...
void *read(void *t) {
  long tid = (long)t;
  long iterals;
  /* Here reader thread just lock and unlock the mutex to ensure mutual
   * exclusion wrt to writer thread only*/
  do {
    iterals = (tid * ITERALS) + 1;
    pthread_mutex_lock(&resource.resrc_mutex);
    ...
    while (iterals) {
      printf("reader thread %ld reads %s, Number of Active readers = %d\n", tid,
             resource.slot[0], resource.Nreader_count);
      sleep(1);
      iterals--;
    }
    pthread_mutex_lock(&resource.Nreader_count_mutex);
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < NREADERS; i++)
    pthread_create(&(reader_threads[i].pthread_handle), &reader_threads[i].attr,
                   read, (void *)i);

  for (i = 0; i < NWRITERS; i++)
    pthread_create(&(writer_threads[i].pthread_handle), &writer_threads[i].attr,
                   write, (void *)NREADERS + i);
