...
recvr_thread id = %d, line_no = %d", sender_thid, recvr_thid, line_no);
        pthread_exit(NULL);
}
 pthread_mutex_unlock(&mutex);
}
...
void *who_am_i(void *arg) {
  ...
  while (++i) {
    ...
    signal(thid, (thid + 1) % THREAD_NUM, cond[(thid + 1) % THREAD_NUM],
           __LINE__);
    wait(thid, cond[thid], __LINE__);
    // sleep(1);
  } // while ends
