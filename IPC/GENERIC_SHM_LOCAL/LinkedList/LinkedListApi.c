...
void reverse_singly_ll(ll_t *ll) {
  ...
  if (GET_NODE_COUNT_SINGLY_LL(ll) == 1)
    return;
  singly_ll_node_t *p1 = GET_HEAD_SINGLY_LL(ll), *p2 = ll->head->next,
                   *p3 = NULL;
  p1->next = NULL;
  do {
    p3 = p2->next;
...
uint32_t ll_get_node_size() { return sizeof(singly_ll_node_t); }
...
void *ll_lookup(void *ds, void *key) {
  if (!ds || !key)
    return NULL;
  ll_t *list = (ll_t *)ds;
...
uint32_t ll_get_key_size(void *ds) { return ((ll_t *)ds)->key_size; }
