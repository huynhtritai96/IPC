...
#define THREAD_NUM 4
#define RESOURCE_CAPACITY                                                      \
  2 // adivisable to keep resource capacity less than THREAD_NUM

typedef struct _pthread_t_ {
  pthread_t pthread_handle;
  int selfid;    // Thread id
  int isWaiting; // False if thread is executing, else true when when Waited
  int blocker_thread_id; // meaningfull only when isWaiting is true, its  a
                         // thread id of the thread which has called the WAIT
                         // for this thread.
  int resume_thread_id;  // meaningfull only when isWaiting is false, its a
                         // thread id of the thread which has called the signal
                         // for this thread.
  pthread_attr_t attr;   // attribute for this thread
  pthread_cond_t cond;   // condition variable for this thread.
  int pass_tkt_count; // +ve value represents the number of SIGNAL accumulated
                      // on this thread, whereas -ve value represents the number
                      // of WAITs accumulated on this thread.
} _pthread_t;

/* Block thread pool Data Structure and Dunctions */
...
static int am_i_in_block_pool(int thid, blocked_pool_t block_pool) {
  int i;
...
static int am_i_in_block_pool_mutex(int thid, blocked_pool_t block_pool) {
  int i;
...
static void init_blocked_pool(blocked_pool_t *block_pool) {
  int i = 0;
...
int get_empty_slot_from_pool(blocked_pool_t block_pool) {
  int i;
...
static int _pool_add(blocked_pool_t *block_pool, int thid) {
  int loc = -1;
  if (thid > THREAD_NUM - 1)
    return -1;
...
static int unblock_thread(blocked_pool_t *block_pool, int thid) {
  int i = 0, loc = -1;
  if (thid > THREAD_NUM - 1)
    return loc;
  pthread_mutex_lock(&block_pool->pool_mutex);
...
/* Block thread pool DS and function ends*/

/*Resource Management API*/

typedef int (*fnptr)(FILE *, char *);

typedef struct SharedResource_t {
  void *resource;
  fnptr access_resource_fn;
  int semaphore;
  blocked_pool_t *block_pool;
  pthread_mutex_t resource_mutex;
  pthread_mutex_t semaphore_mutex;
  int resource_capacity;
} resource_t;

void init_resource(resource_t *resource, fnptr access_resource_fn_fnptr,
                   int resource_cap, void *resource_entity) {
  resource->resource =
      (void *)resource_entity; // assign pointer to some resource here
  resource->access_resource_fn = access_resource_fn_fnptr;
  resource->semaphore = 0;
  resource->block_pool = (blocked_pool_t *)malloc(sizeof(blocked_pool_t));
  pthread_mutex_init(&resource->resource_mutex, NULL);
  pthread_mutex_init(&resource->semaphore_mutex, NULL);
  resource->resource_capacity = resource_cap;
  return;
}

void accessResource_wrapper(resource_t *resource, char *data) {
  pthread_mutex_lock(&resource->resource_mutex);
  resource->access_resource_fn((FILE *)resource->resource, data);
  pthread_mutex_unlock(&resource->resource_mutex);
}

int semaphore_inc(resource_t *resource) {
  pthread_mutex_lock(&resource->semaphore_mutex);
  resource->semaphore++;
  pthread_mutex_unlock(&resource->semaphore_mutex);
  return resource->semaphore;
}

int semaphore_dec(resource_t *resource) {
  pthread_mutex_lock(&resource->semaphore_mutex);
  resource->semaphore--;
  pthread_mutex_unlock(&resource->semaphore_mutex);
  return resource->semaphore;
...
void wait(_pthread_t *thread_to_block, _pthread_t *blocking_thread,
          int line_no) {
  pthread_mutex_lock(&mutex);
  printf("wait called at line no : %d : thread %d is waiting.\n", line_no,
         thread_to_block->selfid);
  ...
  if (pthread_cond_wait(&(thread_to_block->cond), &mutex)) {
    printf("pthread_cond_wait failed, thread id = %d, line_no = %d",
           thread_to_block->selfid, line_no);
...
void signal(_pthread_t *sender_thread, _pthread_t *recvr_thread, int line_no) {
  ...
  if (pthread_cond_signal(&(recvr_thread->cond))) {
    printf("pthread_cond_signal signalling failed, sender_thread id = %d, "
           ...
           sender_thread->selfid, recvr_thread->selfid, line_no);
    pthread_exit(NULL);
  }
...
void pthread_init(_pthread_t *_pthread, int tid, int JOINABLE) {
...
resource_t gl_resource;

int file_writer(FILE *fptr, char *data) {
  if (!fptr)
    return -1;
  fprintf(fptr, "%s", data);
  fflush(fptr);
...
int main(int argc, char **argv) {
  ...
  /*used to synchronise wait and signal calls*/
  pthread_mutex_init(&mutex, NULL);

  /*Initialising global block thread pool*/
  printf("Initializing gl_blocked_th_pool\n");
  init_blocked_pool(gl_resource.block_pool);
  ...
  /* Initialise global resource*/
  printf("Initializing gl_resource\n");
  init_resource(&gl_resource, file_writer, RESOURCE_CAPACITY, (void *)fptr);
  ...
  if (pthread_create(&thread[0].pthread_handle, &thread[0].attr, Master_thread,
                     (void *)0)) {
  ...
  thread_arg th_arg;
  th_arg.resource = &gl_resource;

  for (i = 1; i < THREAD_NUM; i++) {
    th_arg.thid = i;
    if (pthread_create(&thread[i].pthread_handle, &thread[i].attr, Slave_thread,
                       (void *)&th_arg)) {
...
void *Master_thread(void *arg) {
  ...
  while (++i) {
    ...
    case 1: {
      ...
      if (block_thread(gl_resource.block_pool, inp_th_id) == 0) {
        printf("Master thread added thread %d to gl_blocked_th_pool, at index "
               "= %d\n",
               inp_th_id,
               am_i_in_block_pool_mutex(inp_th_id, *gl_resource.block_pool));
      }
      break;
    }
    case 2: {
      ...
      unblock_thread(gl_resource.block_pool, inp_th_id);
...
void *Slave_thread(void *arg1) {
  thread_arg *arg = (thread_arg *)arg1;
  int thid = arg->thid;
  resource_t *resource = arg->resource;
  char data[30];
  int i = 0;

  while (++i) {
    if (am_i_in_block_pool_mutex(thid, *(resource->block_pool)) > -1) {
      printf("Thread %d realised that is has been black listed, hence blocking "
             "itself\n",
             thid);
    ...
    accessResource_wrapper(resource, data);
  } // while ends
