 ...
 *       Filename:  rt.c
 *
 *    Description:  This file implements the routines to work with simple
 * Routing Table
 *
 *        Version:  1.0
 ...
 *       Compiler:  gcc
 *
 *         Author:  Er. Abhishek Sagar, Juniper Networks
 * (https://csepracticals.wixsite.com/csepracticals), sachinites@gmail.com
 *        Company:  Juniper Networks
 *
 *        This file is part of the Inter-Thread Communication distribution
 * (https://github.com/sachinites) Copynext (c) 2019 Abhishek Sagar. This
 ...
 *
 *        visit website : https://csepracticals.wixsite.com/csepracticals for
...
rt_table_t *rt_create_new_rt_table(char *name) {
...
rt_insert_new_entry(rt_table_t *rt, char *dest, char mask, char *gw,
                    char *oif) {
  rt_table_entry_t *rt_table_entry = rt_look_up_rt_table_entry(rt, dest, mask);
  ...
  rt_table_entry = calloc(1, sizeof(rt_table_entry_t));
...
int /*0 on success, -1 on failure*/
rt_delete_rt_entry(rt_table_t *rt, char *dest, char mask) {
...
int /*0 on success, -1 on failure*/
rt_update_rt_entry(rt_table_t *rt, char *dest, char mask, char *new_gw,
                   char *new_oif) {
...
void rt_display_rt_table(rt_table_t *rt) {
  ...
  for (rt_table_entry = rt->head; rt_table_entry;
    ...
    uptime_in_seconds =
        (unsigned int)difftime(curr_time, rt_table_entry->last_updated_time);

    printf("%d. %-18s %-4d %-18s %s\n", i, rt_table_entry->dest,
           rt_table_entry->mask, rt_table_entry->gw, rt_table_entry->oif);
    printf("Last updated : %s\n", hrs_min_sec_format(uptime_in_seconds));
    i++;
  }
}

rt_table_entry_t *rt_look_up_rt_table_entry(rt_table_t *rt, char *dest,
                                            char mask) {
